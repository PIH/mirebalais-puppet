class openmrs (
  ...
  # TODO: will this break things when starting from scratch?
  exec { 'make mirebalais metadata module not mandatory':
    command     => "mysql -u${openmrs_db_user} -p'${openmrs_db_password}' ${openmrs_db} -e 'update openmrs_mirebalais.global_property set property_value=\"false\" where property=\"mirebalaismetadata.mandatory\"'",
    user        => 'root',
    require => Package['pihemr'],
    notify => [ Exec['tomcat-restart'] ]
  }
