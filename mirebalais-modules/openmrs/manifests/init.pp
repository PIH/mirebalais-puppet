class openmrs (
  ...
  file_line { "web_xml_session_config":
    require =>  Package['pihemr'],
    ensure => "present",
    path => "${tomcat_webapp_dir}/${webapp_name}/WEB-INF/web.xml",
    line => "        <session-config>",
    match => "\s*<session-config>"
  }

  file_line { "web_xml_session_timeout":
    require =>  Package['pihemr'],
    ensure => "present",
    path => "${tomcat_webapp_dir}/${webapp_name}/WEB-INF/web.xml",
    line => "            <session-timeout>${session_timeout}</session-timeout>",
    match => "\s*<session-timeout>.*</session-timeout>",
    after => "\s*<session-config>"
  }

  file_line { "web_xml_session_config_close":
    require =>  Package['pihemr'],
    ensure => "present",
    path => "${tomcat_webapp_dir}/${webapp_name}/WEB-INF/web.xml",
    line => "        </session-config>",
    match => "\s*</session-config>",
    after => "\s*<session-timeout>.*"
  }
